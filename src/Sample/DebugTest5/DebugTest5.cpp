...
typedef NTSTATUS (WINAPI* PQUE_INFO_PROC)
(
...
typedef NTSTATUS(WINAPI* PQUE_INFO_THRD)
(
	HANDLE          ThreadHandle,
	...
	while (bRunDbg)
	{
		...
		switch (dwIdx)
		{
...
#ifdef _X86_
				pii->OpOld = SetBreakPoint(pi.hProcess, (PBYTE)ctx.Eip);
			...
			case EVTIDX_PEB:
			{
				...
				DWORD dwSize = 0;
				if (G_PFN_QIP(pi.hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), &dwSize) == 0)
				{
					printf("PPEB: 0x%p\n", pbi.PebBaseAddress);
				}
				THREAD_BASIC_INFORMATION tbi;
				if (G_PFN_QIT(pi.hThread, (THREADINFOCLASS)0, &tbi, sizeof(tbi), &dwSize) == 0)
				{
					printf("PTEB: 0x%p\n", tbi.TebBaseAddress);
#ifdef _X86_
					PBYTE pHdlr = NULL;
					ReadProcessMemory(pi.hProcess, tbi.TebBaseAddress, &pHdlr, sizeof(PBYTE), NULL);
					if (pHdlr != NULL)
					{
						do
						{
							SEH_FRAME sf;
							ReadProcessMemory(pi.hProcess, pHdlr, &sf, sizeof(sf), NULL);
							printf("....Next     : 0x%p\n", sf.Next);
							printf("....Handler  : 0x%p\n", sf.Handler);

							pHdlr = sf.Next;
						} 
						while (pHdlr != (PBYTE)0xFFFFFFFF);
					}
#endif
				}
			}
...
void _tmain(int argc, TCHAR* argv[])
{
	...
	if (hNtDll != NULL)
	{
		G_PFN_QIP = (PQUE_INFO_PROC)GetProcAddress(hNtDll, "ZwQueryInformationProcess");
		G_PFN_QIT = (PQUE_INFO_THRD)GetProcAddress(hNtDll, "NtQueryInformationThread");
	}
	...
	while (true)
	{
		...
		else if (_stricmp(szIn, "peb") == 0)
		{
			if (G_PFN_QIP != NULL)
				SetEvent(ii.DbgEvts[EVTIDX_PEB]);
		}
