...
#define 	UWOP_SPARE_CODE1		6
#define 	UWOP_SPARE_CODE2		7
...
BOOLEAN RtlDispatchException(PEXCEPTION_RECORD ExceptionRecord, PCONTEXT ContextRecord)
{
	...
	// frame pointer.
	ULONG64 HighLimit, LowLimit;
	RtlpGetStackLimits(&LowLimit, &HighLimit);

	CONTEXT ctx;
	RtlpCopyContext(&ctx, ContextRecord);
	ULONG64 ControlPc = (ULONG64)ExceptionRecord->ExceptionAddress;

	ULONG ExceptionFlags = (ExceptionRecord->ExceptionFlags & EXCEPTION_NONCONTINUABLE);

	// Initialize the unwind history table.
	UNWIND_HISTORY_TABLE UnwindTable;
	PUNWIND_HISTORY_TABLE HistoryTable = &UnwindTable;
	...
	do
	{
		...
		ULONG64 ImageBase;
		PRUNTIME_FUNCTION FunctionEntry = RtlLookupFunctionEntry(ControlPc, &ImageBase, HistoryTable);
		...
		if (FunctionEntry != NULL)
		{
			...
			PVOID	HandlerData;
			PEXCEPTION_ROUTINE ExceptionRoutine = RtlVirtualUnwind
			(
				UNW_FLAG_EHANDLER, ImageBase, ControlPc, FunctionEntry,
			...
			if (ExceptionRoutine != NULL) 
			{
				...
				do 
				{
					...
					dc.HistoryTable		= HistoryTable;
					dc.ScopeIndex		= ScopeIndex;

					EXCEPTION_DISPOSITION Disposition = RtlpExecuteHandlerForException
					(
						ExceptionRecord, EstablisherFrame, ContextRecord, &dc
					);
					...
					switch (Disposition) 
					{
						...
						case ExceptionNestedException:
							ExceptionFlags |= EXCEPTION_NESTED_CALL;
							if (dc.EstablisherFrame > NestedFrame) 
							{
								NestedFrame = dc.EstablisherFrame;
							}
						break;
						...
						case ExceptionCollidedUnwind:
							ControlPc		 = dc.ControlPc;
							ImageBase		 = dc.ImageBase;
							FunctionEntry	 = dc.FunctionEntry;
							EstablisherFrame = dc.EstablisherFrame;
							RtlpCopyContext(&ctx, dc.ContextRecord);

							ctx.Rip			 = ControlPc;
							ExceptionRoutine = dc.LanguageHandler;
							HandlerData		 = dc.HandlerData;
							HistoryTable	 = dc.HistoryTable;
							ScopeIndex		 = dc.ScopeIndex;
							Repeat			 = TRUE;
						break;
	...
	while (RtlpIsFrameInBounds(&LowLimit, (ULONG64)ctx.Rsp, &HighLimit) == TRUE);
...
)
{
	...
	RtlUnwindEx(TargetFrame, TargetIp, 
		ExceptionRecord, ReturnValue, &ContextRecord, NULL);
}
...
)
{
	...
	PCONTEXT CurrentContext = OriginalContext;
	...
	ULONG ExceptionFlags = EXCEPTION_UNWINDING;
	if (TargetFrame == NULL) 
	{
	...
	do
	{
		...
		PCONTEXT PreviousContext = &LocalContext;
		if (FunctionEntry != NULL)
		{
			...
			PEXCEPTION_ROUTINE ExceptionRoutine = RtlVirtualUnwind
			(
				UNW_FLAG_UHANDLER, ImageBase, ControlPc, FunctionEntry,
				PreviousContext, &HandlerData, &EstablisherFrame, NULL
			);
			...
			else if (ExceptionRoutine != NULL)
			{
				...
				do
				{
					...
					// Set the dispatcher context and call the termination handler.
					dc.ControlPc		= ControlPc;
					dc.ImageBase		= ImageBase;
					dc.FunctionEntry	= FunctionEntry;
					dc.EstablisherFrame = EstablisherFrame;
					dc.ContextRecord	= CurrentContext;
					dc.LanguageHandler	= ExceptionRoutine;
					dc.HandlerData		= HandlerData;
					dc.HistoryTable		= HistoryTable;
					dc.ScopeIndex		= ScopeIndex;
					EXCEPTION_DISPOSITION Disposition = RtlpExecuteHandlerForUnwind
					(
						ExceptionRecord, EstablisherFrame, CurrentContext, &dc
					);
					...
					switch (Disposition)
					{
						...
						case ExceptionCollidedUnwind:
							...
							ImageBase = dc.ImageBase;
							FunctionEntry = dc.FunctionEntry;
							RtlpCopyContext(OriginalContext, dc.ContextRecord);
							...
							RtlVirtualUnwind
							(
								UNW_FLAG_NHANDLER, ImageBase, ControlPc, FunctionEntry,
								PreviousContext, &HandlerData, &EstablisherFrame, NULL
							);
			...
			{
				// If the target frame has not been reached, then swap context pointers.
				if (EstablisherFrame != (ULONG64)TargetFrame)
				{
					TempContext = CurrentContext;
					CurrentContext = PreviousContext;
...
)
{
	...
	if ((UnwindInfo->Flags & UNW_FLAG_CHAININFO) != 0) 
	{
		...
		return RtlpUnwindPrologue(ImageBase, ControlPc, FrameBase, 
			FunctionEntry, ContextRecord, ContextPointers);
	}
	else 
	{
...
)
{
	...
	else if ((NextByte[0] == JMP_IMM8_OP) || (NextByte[0] == JMP_IMM32_OP))
	{
			...
			BranchTarget >= FunctionEntry->EndAddress) 
		{
			...
			if ((PrimaryFunctionEntry == NULL) || (BranchTarget == PrimaryFunctionEntry->BeginAddress))
				InEpilogue = TRUE;
		}
...
None.
--*/
DECLSPEC_NOINLINE VOID RtlpGetStackLimits(OUT PULONG64 LowLimit, OUT PULONG64 HighLimit)
...
--*/
BOOLEAN RtlpIsFrameInBounds(IN OUT PULONG64 LowLimit, IN ULONG64 StackFrame, IN OUT PULONG64 HighLimit)
{
	...
	if ((StackFrame < *LowLimit) || (StackFrame >= *HighLimit))
	{
		if (KeIsExecutingLegacyDpc() == FALSE) 
		{
			...
			if (Control->Previous.StackBase != 0) 
			{
				...
				if ((StackFrame >= LocalLow) && (StackFrame < LocalHigh)) 
				{
					*LowLimit = LocalLow;
					*HighLimit = LocalHigh;
					return TRUE;
...
--*/
FORCEINLINE PRUNTIME_FUNCTION RtlpConvertFunctionEntry(IN PRUNTIME_FUNCTION FunctionEntry, IN ULONG64 ImageBase)
{
	//
...
--*/
PRUNTIME_FUNCTION RtlLookupFunctionEntry(IN ULONG64 ControlPc, OUT PULONG64 ImageBase, 
	IN OUT PUNWIND_HISTORY_TABLE HistoryTable OPTIONAL)
{
	...
	if ((ARGUMENT_PRESENT(HistoryTable)) && (HistoryTable->Search != UNWIND_HISTORY_TABLE_NONE)) 
	{
		...
		// Search the dynamic unwind history table if there is a chance of a match.
...
PRUNTIME_FUNCTION RtlpSameFunction(IN PRUNTIME_FUNCTION FunctionEntry, IN ULONG64 ImageBase, IN ULONG64 ControlPc)
{
	...
	TargetFunctionEntry = RtlLookupFunctionEntry(ControlPc, &TargetImageBase, NULL);
