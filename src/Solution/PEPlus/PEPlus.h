...
#define SIZE_OF_UWND_INFO	(sizeof(UNWIND_INFO) - sizeof(UNWIND_CODE))
...
{
	ULONG Count;
	C_SCOPE_TABLE_ENTRY ScopeRecord[1];
...
typedef struct _s_FuncInfo
{
	...
	DWORD	DispUnwindMap;		// RVA of the unwind map
	UINT	TryBlocks;			// count of try blocks
	DWORD	DispTryBlockMap;	// RVA of the try block array 
	UINT	IPMapEntries;		// count of IP-to-state entries
	DWORD	DispIPtoStateMap;	// RVA of the IP-to-state array
	...
	int		EHFlags;			// flags
} S_FUNC_INFO, *PS_FUNC_INFO;

typedef struct _iptoStateMapEntry 
{
	__int32		Ip;		// Image relative offset of IP
	__int32		State;	//__ehstate_t	
} IptoStateMapEntry, *PIptoStateMapEntry;
...
typedef const struct _s_TryBlockMapEntry
{
	...
	int	CatchCnt;			// Number of entries in array
...
{
	DWORD		Adjectives;		// Handler Type adjectives (bitfield)
...
typedef struct EHExceptionRecord 
{
	...
	PEXCEPTION_RECORD	ExceptionRecord; // extra record (not used)
	PVOID				ExceptionAddress; // Address at which exception occurred
	DWORD				NumberParameters; // Number of extended parameters. (=4)
	struct EHParameters 
	{
		DWORD			magicNumber; // = EH_MAGIC_NUMBER1
		PVOID			pExceptionObject; // Pointer to the actual object thrown
		DWORD			pThrowInfo; // Description of thrown object
		PVOID			pThrowImageBase; // Image base of thrown object
	} params;
} EHExceptionRecord;
////////////////////////////////////////////////////////////////////////////////////////////////////
...
public:
	...
	static DWORD GetImageSize(PBYTE pImgBase);
	...
	static PIMAGE_PDB_INFO GetPdbInfo(PBYTE pImgBase, DWORD* pdwTimeStamp = NULL, bool bIsPE = true);
...
inline DWORD PEPlus::GetImageSize(PBYTE pImgBase)
{
	PIMAGE_NT_HEADERS pnh = (PIMAGE_NT_HEADERS)GET_NT_HDRPTR(pImgBase);
	if (Is32bitPE(pImgBase))
		return ((PIMAGE_OPTIONAL_HEADER32)&pnh->OptionalHeader)->SizeOfImage;
	else
		return ((PIMAGE_OPTIONAL_HEADER64)&pnh->OptionalHeader)->SizeOfImage;
}
