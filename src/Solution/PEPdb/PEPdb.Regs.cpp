...
//
static int cmpIa64regSz(const void *pv1, const void *pv2) 
{
...
//
PCWSTR PEPdb::ToStr_C7REG(USHORT uReg, DWORD dwMachType)
{
	...
	swprintf_s(wszRegNum, L"???(0x%x)", uReg);
	...
	switch (dwMachType)
	{
		...
		case CV_CFL_80486:
		case CV_CFL_PENTIUM:
			if (uReg < (sizeof(rgRegX86) / sizeof(*rgRegX86)))
				return(rgRegX86[uReg]);
			return wszRegNum;
		case CV_CFL_ALPHA:
			if (uReg < (sizeof(rgRegAlpha) / sizeof(*rgRegAlpha)))
				return(rgRegAlpha[uReg]);
			return wszRegNum;
		case CV_CFL_MIPSR4000:
		case CV_CFL_MIPS16:
			if (uReg < (sizeof(rgRegMips) / sizeof(*rgRegMips)))
				return(rgRegMips[uReg]);
			return wszRegNum;
		case CV_CFL_M68000:
		case CV_CFL_M68010:
		case CV_CFL_M68020:
		case CV_CFL_M68030:
		case CV_CFL_M68040:
			if (uReg < (sizeof(rgReg68k) / sizeof(*rgReg68k)))
				return(rgReg68k[uReg]);
			return wszRegNum;
		case CV_CFL_PPC601:
		case CV_CFL_PPC603:
		case CV_CFL_PPC604:
		case CV_CFL_PPC620:
			if (uReg < (sizeof(rgRegPpc) / sizeof(*rgRegPpc)))
				return(rgRegPpc[uReg]);
			return wszRegNum;
		case CV_CFL_SH3:
			if (uReg < (sizeof(rgRegSh) / sizeof(*rgRegSh)))
				return(rgRegSh[uReg]);
			return wszRegNum;
		case CV_CFL_ARM3:
		case CV_CFL_ARM4:
		case CV_CFL_ARM4T:
			if (uReg < (sizeof(rgRegArm) / sizeof(*rgRegArm)))
				return(rgRegArm[uReg]);
			return wszRegNum;
		case CV_CFL_IA64:
		{
			MapIa64Reg *p;
			MapIa64Reg  m = { (CV_HREG_e)uReg };
			p = (MapIa64Reg*)bsearch(&m, mpIa64regSz,
				sizeof(mpIa64regSz) / sizeof(*mpIa64regSz),
				sizeof(MapIa64Reg), cmpIa64regSz);
			if (p)
				return p->wszRegName;
			else
				return wszRegNum;
		...
		case CV_CFL_AMD64:
			if (uReg < sizeof(rgRegAMD64) / sizeof(*rgRegAMD64))
				return rgRegAMD64[uReg];
			else
				return wszRegNum;
		default:
			return wszRegNum;
