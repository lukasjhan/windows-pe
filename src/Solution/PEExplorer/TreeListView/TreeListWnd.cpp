...
typedef struct
	...
	BYTE		bCallback;									// Sind BitFld f? Callbacks
...
NMTVDISPINFO	sInfo;
	...
	if(uFlags&TVIF_TEXT)
	{
		...
		uTextSize	= (UINT)_tcslen(sInfo.item.pszText);
...
WNDPROC			pProc;
	...
	pProc = (WNDPROC)GetWindowLongPtr(hWnd, MYGWL_USERDATA);
	if(uMsg==WM_KEYDOWN)
		{
		if(wParam==VK_RETURN)
			{
...
int				iLen;
	...
	pData = (TreeListData*)GetWindowLongPtr(hWnd,MYGWL_USERDATA);
	...
	if(uMsg==WM_PAINT)
		...
		iLen=(INT)CallWindowProc(pProc,hWnd,WM_GETTEXT,sizeof(cText)/sizeof(TCHAR),(LPARAM)cText);
...
static void CrateToolTip(TreeListData *pData)
{
	...
	pData->hToolTip = CreateWindow(_T("STATIC"), _T(""), WS_POPUP | WS_BORDER | WS_CLIPSIBLINGS, 0, 0, 0, 0, pData->hWnd, NULL, NULL, NULL);
	pData->pToolTipProc = (WNDPROC)GetWindowLongPtr(pData->hToolTip,MYGWL_WNDPROC);

	SetWindowLongPtr(pData->hToolTip,MYGWL_USERDATA,(LPARAM)pData);
	SetWindowLongPtr(pData->hToolTip,MYGWL_WNDPROC ,(LPARAM)ToolTipProc);
...
static int TreeListToggleItem(TreeListData *pData,unsigned uItem,unsigned uAddFlags)
{
	...
	if(pData->uSelectedItem)								// Ist der ausgew?lten Eintrag sichtbar ?
	{
		...
		if (pEntry != NULL && !pEntry->uShowPos)
		{
...
unsigned	uRet;
	...
	UNLOCK(pData);
	uRet = (UINT)SendNotify(pData, &sNotify.hdr);
	LOCK(pData);
...
//*****************************************************************************
	...
	UNLOCK(pData);
	uRet = (UINT)SendNotify(pData, &sNotify.hdr);					// Anfragen ob die Zweige umgeklappt werden d?fen
	LOCK(pData);
...
//******************** Den neuen Eintrag erzeugen *****************************
	...
	if(uBits&TVIF_TEXT)										// Text einf?en
		...
		if(pInsert->item.pszText==LPSTR_TEXTCALLBACK)
			...
			pNew->uTextSize	= (WORD)_tcslen(pInsert->item.pszText);
...
//******************** Eintrage einf?en **************************************
	...
	case U(TVI_SORT):										// Alphapetisch einf?en
			...
			for(;uItem;uItem=pItems[uItem]->uNextItem)		// Suche den Eintrag
				...
				if(_tcsicmp(pNew->pText,pTextTemp)<0)break;
				}
		...
		if(uSub>0)
			...
			if(uBits&TVIF_TEXT)								// Einen neuen Text einstellen
				...
				if(pItem->pszText==LPSTR_TEXTCALLBACK)
					...
					uLen = (UINT)_tcslen(pItem->pszText);
...
int				iTemp;
	...
	if(!pData->hEdit)										// Editfenster neu erzeugen
		...
		switch(uBits&3)
			{
		case 1:	pData->hEdit = CreateWindow(_T("COMBOBOX"), NULL, WS_BORDER | WS_CHILD | ES_AUTOHSCROLL | ES_LEFT | CBS_DROPDOWN, 0, 0, 0, 0, pData->hWnd, NULL, NULL, NULL);
				pData->uEditMode=1;
				break;

		case 2:	pData->hEdit = CreateWindow(_T("COMBOBOX"), NULL, WS_BORDER | WS_CHILD | ES_AUTOHSCROLL | ES_LEFT | CBS_DROPDOWNLIST, 0, 0, 0, 0, pData->hWnd, NULL, NULL, NULL);
				pData->uEditMode=2;
				break;
		...
		pProc = (WNDPROC)GetWindowLongPtr(pData->hEdit,MYGWL_WNDPROC);

		SetWindowLongPtr(pData->hEdit,MYGWL_USERDATA,(LPARAM)pProc);
		SetWindowLongPtr(pData->hEdit,MYGWL_WNDPROC ,(LPARAM)EditProc);
		...
		while(hWnd)
			{
			pProc = (WNDPROC)GetWindowLongPtr(hWnd,MYGWL_WNDPROC);
			SetWindowLongPtr(hWnd,MYGWL_USERDATA,(LPARAM)pProc);
			SetWindowLongPtr(hWnd,MYGWL_WNDPROC ,(LPARAM)EditProc);
	...
	if(uSub==0)												// Haupteintrag bearbeiten
		...
		if(pEntry->bCallback&TVIF_TEXT)
			{
			CallbackEntry(pData,pEntry,uItem,TVIF_TEXT,iTemp,uSize,(PCTSTR&)pText);
			hDc=GetDC    (pData->hWnd);
			SelectObject (hDc,hFont);
			DrawText     (hDc,pText,0,&sRect,DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX|DT_CALCRECT);
			ReleaseDC    (pData->hWnd,hDc);
		...
		if(!pExtra)
			...
			if(pExtra->bCallback&TVIF_TEXT)
				{
				CallbackExtra(pData,pEntry,pExtra,uItem,uSub,TVIF_TEXT,iTemp,uSize,(PCTSTR&)pText);
				hDc=GetDC    (pData->hWnd);
				SelectObject (hDc,hFont);
				DrawText     (hDc,pText,0,&sRect,DT_LEFT|DT_VCENTER|DT_SINGLELINE|DT_NOPREFIX|DT_CALCRECT);
				ReleaseDC    (pData->hWnd,hDc);
...
static LRESULT CALLBACK TreeListProc(HWND hWnd,UINT uMsg,WPARAM wParam,LPARAM lParam)
{
	...
	case WM_SIZE:	// Die Fenstergröße wurde ver?dert
		...
		if(uVal!=pData->uSizeX)
		{
			...
			MoveWindow(pData->hHeader, -(int)pData->uScrollX, 0,
				pData->uSizeX + pData->uScrollX-1, pData->uStartPixel, FALSE);
			UpdateScrollX(pData);
		}
	...
	case WM_MOUSEMOVE:										// Gab es eine Mausbewegung
			...
			if (pData->uStyleEx & TVS_EX_SEND_HOVER)
			{
				if (!pData->cIsEnabled)
					return 0;
				...
				sInfo.pt.y = HIWORD(lParam);
				TreeListHitTest(pData, &sInfo);
				if (sInfo.hItem != NULL && (sInfo.flags & TVHT_ONSUBLABEL))
				{
					unsigned uItem = U(sInfo.hItem);
					if (pData->uHoverItem == uItem)
					{
						UNLOCK(pData);
						return 0;
					}

					BaseItem* pEntry = pData->pTreeItems[uItem];
					if (pEntry == NULL)
					{
						UNLOCK(pData);
						return 0;
					...
					nmtv.hdr.code = NM_HOVER;
					nmtv.itemNew.mask = TVIF_HANDLE | TVIF_PARAM | TVIF_STATE | TVIF_SUBNUMBER;
					nmtv.itemNew.hItem = sInfo.hItem;
					nmtv.itemNew.stateMask = 0xFFFFFFFF;
					nmtv.itemNew.state = pEntry->uState;
					nmtv.itemNew.lParam = pEntry->lParam;
					nmtv.itemNew.cChildren = TVHT_SUBTOCOL(sInfo.flags);
					nmtv.itemNew.pszText = (LPTSTR)-1;
					nmtv.itemNew.cchTextMax = -1;
					nmtv.ptDrag.x = LOWORD(lParam);
					nmtv.ptDrag.y = HIWORD(lParam);
					if (pData->uHoverItem != 0)
					{
						pEntry = pData->pTreeItems[pData->uHoverItem];
						if (pEntry != NULL)
						{
							nmtv.itemOld.mask = TVIF_HANDLE | TVIF_PARAM | TVIF_STATE | TVIF_SUBNUMBER;
							nmtv.itemOld.hItem = (HTREEITEM)pData->uHoverItem;
							nmtv.itemOld.stateMask = 0xFFFFFFFF;
							nmtv.itemOld.state = pEntry->uState;
							nmtv.itemOld.lParam = pEntry->lParam;
							nmtv.itemOld.cChildren = TVHT_SUBTOCOL(sInfo.flags);
							nmtv.itemOld.pszText = (LPTSTR)-1;
							nmtv.itemOld.cchTextMax = -1;
						}
					}
					SendNotify(pData, &nmtv.hdr);
					pData->uHoverItem = uItem;
				}
				else
				{
					if (pData->uHoverItem != 0)
					{
						unsigned uItem = U(pData->uHoverItem);
						BaseItem* pEntry = pData->pTreeItems[uItem];
						if (pEntry == NULL)
						{
							UNLOCK(pData);
	...
	case TVM_GETEXTENDEDSTYLE:								// Abfragen der erweiterten Style-BitFld

			pData = GetHandle(hWnd);
			return pData->uStyleEx;

	case TVM_SETEXTENDEDSTYLE:								// Einstellen der erweiterten Style-BitFld
			...
			if((unsigned)lParam<=pData->uTreeItemsMax)
				...
				if(pEntry)
					...
					case TVE_COLLAPSE:	if(pEntry->uState&TVIS_EXPANDED)
											{
												lRet = (TreeListToggleItem(pData, (UINT)lParam, 0)) ? 0 : 1;
											}
										else{
	...
	case TVM_SETITEMBKCOLOR:								// Hintergrundfarbe eines Eintrages ?dern
			...
			LOCK(pData);
			lRet = TreeListSetItemColor(pData, (unsigned)wParam & 0xFFFFFF, (UINT)wParam >> 24, (COLORREF)lParam, 0);
			UNLOCK(pData);
	...
	case TVM_SETITEMTEXTCOLOR:								// Textfarbe eines Eintrages ?dern
			...
			LOCK(pData);
			lRet = TreeListSetItemColor(pData, (unsigned)wParam & 0xFFFFFF, (UINT)wParam >> 24, (COLORREF)lParam, 1);
			UNLOCK(pData);
	...
	case TVM_SETUSERDATASIZE:								// Einstellen der Gre der User-Daten
			...
			if(pData->uTreeItemsCount>0)
				...
				pData->uUserDataSize = (UINT)lParam;
				lRet = (UINT)lParam;
				}
