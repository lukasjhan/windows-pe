...
int CClassView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	const DWORD dwViewStyle = WS_CHILD | WS_VISIBLE | TVS_HASLINES | 
		TVS_LINESATROOT | TVS_HASBUTTONS | WS_CLIPSIBLINGS | WS_CLIPCHILDREN;
	...
	// 이미지를 로드합니다.
	m_tbMain.Create(this, AFX_DEFAULT_TOOLBAR_STYLE, IDR_SORT);
	m_tbMain.LoadToolBar(IDR_SORT, 0, 0, TRUE /* 잠금 */);

	OnChangeVisualStyle();

	m_tbMain.SetPaneStyle(m_tbMain.GetPaneStyle() | CBRS_TOOLTIPS | CBRS_FLYBY);
	m_tbMain.SetPaneStyle(m_tbMain.GetPaneStyle() & ~(CBRS_GRIPPER | CBRS_SIZE_DYNAMIC |
	...
	m_tbMain.ReplaceButton(ID_SORT_MENU, 
		CClassViewMenuButton(menuSort.GetSubMenu(0)->GetSafeHmenu()));

	CClassViewMenuButton* pButton =  DYNAMIC_DOWNCAST
		(CClassViewMenuButton, m_tbMain.GetButton(0));
	if (pButton != NULL)
	{
...
void CClassView::FillClassView()
{
	...
	m_wndClassView.InsertItem(_T("~CFakeAppFrame()"), 3, 3, hClass);
	m_wndClassView.InsertItem(_T("m_mbMain"), 6, 6, hClass);
	m_wndClassView.InsertItem(_T("m_tbMain"), 6, 6, hClass);
	m_wndClassView.InsertItem(_T("m_sbMain"), 6, 6, hClass);
...
void CClassView::AdjustLayout()
{
	...
	m_wndClassView.SetWindowPos(NULL, rc.left + 1, rc.top + cyTlb + 1, rc.Width() - 2, rc.Height() - cyTlb - 2, SWP_NOACTIVATE | SWP_NOZORDER);
}
...
void CClassView::OnSort(UINT id)
{
	...
	CClassViewMenuButton* pButton =  DYNAMIC_DOWNCAST(CClassViewMenuButton, m_tbMain.GetButton(0));

	if (pButton != NULL)
	{
		pButton->SetImage(GetCmdMgr()->GetCmdImage(id));
		m_tbMain.Invalidate();
		m_tbMain.UpdateWindow();
	}
}
...
void CClassView::OnChangeVisualStyle()
{
	...
	UINT uiBmpId = G_PE_APP.m_bHiColorIcons ? IDB_CLASS_VIEW_24 : IDB_CLASS_VIEW;
	...
	m_tbMain.LoadBitmap(G_PE_APP.m_bHiColorIcons ? IDB_SORT_24 : IDR_SORT, 0, 0, TRUE /* 잠금 */);
}
